...
> **현재 위치:** Phase 1 진행 중. **Task 1~7 완료.** Task 4는 BigQuery 물리 스토리지 2행이 확인 대기이고, Task 8 실제 실행 전에 필수다. 다음은 Task 8(파이프라인 + 실제 실행)이고, 사용자 지시를 기다린다(규칙 10). Phase별 진행계획은 `docs/phases/`에 있다.
> **마지막 갱신:** 2026-09-11
...
| Phase 1: 가격·TCO·대시보드 | **진행 중** (Task 7/8 완료) | 테스트 28/29 통과. 판정 미리보기: T1 기각(미국 민감·서울 견고), T2 지지(39.5%p). 임시 디자인 대시보드 완성(T1 두 리전 표시) | `docs/phases/phase-1-tco-dashboard.md`, `docs/plans/phase1-tco-dashboard.md` |
| Phase 2: 자동 수집·변동 감지·게시·Sheets 적재 | 대기 | Sheets 적재 방식 확정, 전용 새 GCP 프로젝트 결정. **대시보드를 만들기 전에 멈추고 사용자 템플릿을 받는다(2-5a)** | `docs/phases/phase-2-automation-publish.md` |
...
**결정·확인 대기**
1. **BigQuery 물리 스토리지 2행 확인(Task 8 실제 실행 전 필수).** 스토리지 섹션의 "활성 물리적 스토리지"다. 기대값은 US(us) $0.000054795, 서울 $0.000071233 per GiB-시간이다.
2. Task 8 착수 지시
3. (Phase 2-5a) 마음에 드는 대시보드 템플릿
4. LinkedIn 방식: Phase 3에서 결정한다. A' 공식 공유 링크를 추천한다.

**다음 행동:** 사용자가 지시하면 Task 8을 한다. `pipeline.py`를 작성하고 테스트한 뒤 실제로 실행한다. 1번이 확인되지 않았으면 코드와 오프라인 테스트까지만 하고, 실제 실행 단계에서 멈춰 보고한다. 끝나면 보고하고 멈춘다.
...
| `docs/phases/phase-1-tco-dashboard.md` | Phase 1 진행계획. Task 표, 가격 확인 상태, 가정 검토 결과, T1·T2 판정 결과, 대시보드 구성, 사용자 할 일, 위험, 진행 기록 |
| `docs/phases/phase-2-automation-publish.md` | Phase 2 진행계획. 자동 수집, 이벤트 기준, 승인 흐름, Sheets 적재(2-2b)와 준비 방법(2-0c), **디자인 게이트(2-5a)**, 위험 |
...
| `docs/plans/phase1-tco-dashboard.md` | Phase 1 코드 단위 상세 계획(Task 1~8). 맨 위에 진행 현황과 계획 이후 변경점(물리 스토리지, T1 두 리전)이 있다 |
...
[BigQuery 가격 페이지 가격 문자열 지문 비교 (Phase 2, 약관 허용)]              └ 있음 ─┬→ 템플릿 글 초안 → PR
  → 사용자가 브라우저로 확인 → 수동 가격표 PR                                           │        │
                                                                                         │  [사용자가 PR 머지 = 승인]
                                                                                         │        │
                                                                                         │  사이트 빌드(사용자 템플릿 기반 디자인) → GitHub Pages → LinkedIn (방식 결정 대기)
...
- [ ] **Task 4 (1-1, 1-4 변경) 수동 가격표 + 확인 게이트.** 코드 완료(커밋 `c8da16f`), 확인 10행 중 8행 완료
  ...
  - [ ] (U) **BigQuery 물리 스토리지 2행 확인**(가정 검토로 논리 → 물리로 변경). Task 8 실제 실행 전에 필수다.
- [x] **Task 5 (1-5) 워크로드 가정 + 월 비용 계산.** 완료, 커밋 `56e95fb`, 가정 검토 반영 `529fbc4`
  - 테스트 7개 통과(누적 20개).
  - 가정 검토: 스토리지 기준을 "압축(물리) 후 10TB"로 통일했고, BigQuery는 물리 과금을 쓴다. W3 스캔량은 논리 바이트 기준이다.
- [x] **Task 6 (1-6) T1 순위·민감도, T2 서울 프리미엄 판정.** 완료, 커밋 `7126385`
  - 테스트 6개 통과(누적 26개).
  - 미리보기: T1은 미국·서울 모두 기각(미국 민감, 서울 견고), T2는 지지(39.5%p)
- [x] **Task 7 (1-7) 정적 대시보드(기능 확인용 임시 디자인).** 완료
  - 사용자 결정에 따라 T1을 **미국·서울 두 리전 모두** 표시한다. `render()`는 `t1_by_region`을 받는다.
  - 테스트 2개 통과(누적 28개).
  - 실제 데이터로 렌더링해 점검했다: T1 행 2개, 차트 3개, 표 5개.
- [ ] Task 8 (1-8) `pipeline.py` 실제 실행과 검증. **다음 차례**
  - pytest **29개** 통과
  - 두 리전 T1을 `render()`에 넘긴다.
  - (U) 화면 확인, 가격 3개 대조
  - 물리 스토리지 2행이 확인돼야 실제 실행이 된다.
...
- [ ] 2-5 (C) `.github/workflows/publish.yml`: main에 push되면 사이트와 아카이브를 빌드해 Pages에 배포(2-5a 디자인 반영 후)
- [ ] 2-6 (C) 가격 변경 감지와 알림
...
- 가격 실측값과 서울 프리미엄 표는 `docs/02_phase0_report.md`와 Phase 1 진행계획의 "T1·T2 판정 결과"에 있다.
...
- plotly는 계획 기준(5.x)보다 새 버전(7.0.0)이 설치됐다. 그래도 필요한 API(`get_plotlyjs_version`, `to_html`, `add_bar`)는 동작하고, JS 4.0.0 CDN 파일도 200 응답(약 4.3MB)을 확인했다. Task 7 렌더링에서도 정상이었다 [확인].
...
| 2026-09-11 | **가정 검토 결과: 스토리지 기준을 "압축(물리) 후 10TB"로 통일하고, BigQuery는 active physical storage 단가를 쓴다.** W3 스캔량은 논리 바이트 기준이라고 명시한다 | 사용자가 검토를 위임했고, 사용자 승인 후 커밋했다(`529fbc4`). BigQuery 논리 과금만 압축 전 기준이라 비용이 과소평가됐다. 이 변경으로 W3 1위가 BigQuery에서 Redshift로 바뀌었다 |
| 2026-09-11 | **대시보드에 T1을 미국·서울 두 리전 모두 표시한다**(Task 7) | 사용자 결정. 미국은 민감, 서울은 견고로 리전마다 결론의 확실성이 달랐다. 테스트는 1개에서 2개로 늘어, Phase 1 합계가 29개가 됐다 |
...
- **2026-09-11:** Task 5 코드를 완료했다(커밋 `56e95fb`).
  ...
  - 테스트는 누적 20개가 통과했다.
- **2026-09-11:** 사용자 확인을 반영하고 가정을 검토했다(커밋 `529fbc4`).
  ...
  - 사용자가 변경을 승인해 커밋했다.
- **2026-09-11:** Task 6을 완료했다(커밋 `7126385`).
  - `config/thresholds.yaml`을 만들고, 판정 함수 4개를 TDD로 추가했다(ImportError로 실패 확인 → 구현 → 통과).
  - 테스트는 누적 26개가 통과했다.
  - 실제 가격 기준 판정(미리보기)
    - T1은 미국·서울 모두 기각(전 시나리오 Redshift 1위). 미국은 민감(Snowflake ×0.5면 역전), 서울은 견고
    - T2는 지지(39.5%p)
- **2026-09-11:** Task 7을 완료했다.
  - 사용자 결정에 따라 T1을 두 리전 모두 표시하도록 `render()`가 `t1_by_region`을 받게 바꿨다.
  - `publish/render_site.py`와 템플릿을 TDD로 구현했다(ImportError로 실패 확인 → 구현 → 통과).
  - 테스트는 누적 28개가 통과했다.
  - 실제 데이터로 렌더링해 점검했다: T1 행 2개, 차트 3개, 표 5개.
  - 사용자 지시에 따라 규칙 13(공개 대시보드는 사용자 템플릿 기반, Phase 2-5a에서 멈춤)을 추가했다.
  - Task 8 지시를 기다린다.
