...
16. **사용자에게 보이는 한국어는 격식 있는 문어체로 쓴다** (2026-09-12 사용자 지시).
...
- [ ] 3-1 (C) 브라우저로 굽기 실측 `publish/browser.py`
- [ ] 3-2 (C) 카드 데이터와 게시문 `publish/card_data.py`, `publish/render_linkedin.py`
- [ ] 3-3 (C) 카드 HTML 템플릿과 굽기 → (U) 카드 실물 확인
- [ ] 3-4 (C) 파이프라인 연결 `pipeline.py --cards`
- [ ] 3-5 (C) 워크플로 연결(검토 PR에 카드 표시, 머지 후 게시)
- [ ] 3-6 (C) 소개 카드 5장 → (U) 문구 확인
- [ ] 3-7 (C) 종단 검증
- [ ] 3-8 (U) **첫 게시.** 사용자가 LinkedIn에 직접 올린다
...
- **2026-09-12:** 사용자가 화면 문구를 지적했다. 판정 용어가 무슨 뜻인지 알 수 없고, "빌드일"은 개발자 용어이며, "미국과 서울"은 층위가 어긋난다는 지적이다. `/humanize-korean`으로 화면 문구 전체를 다시 썼다.
  ...
  - 검토 보고서 템플릿도 같은 용어로 맞췄다. 규칙 16으로 못 박았다.
