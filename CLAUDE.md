...
**다음 행동:** 3-7 계획이 승인됐다. Step 0 새 문안(`docs/scripts/review-pr-script.md` 5~7절, `docs/scripts/revise-request-script.md`)을 작성·점검했고 **문안 승인을 기다린다.** 승인 뒤 구현하고, push·실측(규칙 4)에서 한 번 더 승인을 받는다. 로컬에만 있는 기록 커밋은 다음 push 때 함께 올린다.
...
- [ ] 3-7 (C) **문구 반려 재작성 루프**(규칙 16). `revise-copy` 라벨 → 재작성 요청 → humanize-korean → 같은 PR 갱신. 계획 승인(2026-09-15), 새 문안 승인 대기
- [ ] 3-8 (C) 소개 카드 구현
- [ ] 3-9 (C) 종단 검증
