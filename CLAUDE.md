...
| `docs/plans/phase3-card-design.md` | 카드뉴스 디자인 적용 계획(규칙 13 게이트). 템플릿 분석, 디자인 토큰, 장별 매핑, 넘치는 항목 처리 |
| `docs/plans/phase3-linkedin-cardnews.md` | Phase 3 코드 단위 상세 계획(Task 1~8). 결정 D7~D13, 테스트, 산출물 폴더 |
...
| 2026-09-12 | **D13** 넘치는 항목은 불릿 5줄로 자르고 `외 N건`으로 적는다. `N`은 `events["display"]`에 기록한다 | 숫자 검사 규칙을 약화시키지 않는다. 보강된 값이 `events.json`에 남아 추적된다 |
| 2026-09-12 | **D14** 문구 반려는 검토 PR의 `revise-copy` 라벨로 받는다. 워크플로는 재작성 요청 Issue까지 만들고, 재작성은 Claude가 `/humanize-korean`으로 수행해 같은 PR을 갱신한다. 상한 3회 | 사용자 지시. `/humanize-korean`은 Claude 스킬이라 Actions 안에서 실행되지 않는다 [확인]. 전면 자동화는 Anthropic API 키가 필요하며, 이 프로젝트는 키를 쓰지 않는 방향으로 왔다 |
