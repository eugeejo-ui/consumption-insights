...
**다음 행동:** 3-8 세부 계획(`docs/tasks/phase3/task-8-intro-cards.md`)을 작성했고 **승인을 기다린다.** 결정 D21(소개 카드는 로컬에서 굽고 실물 승인 후 push)·D22(카드 글꼴 목록에 Noto Sans KR 추가)를 함께 묻는다. 승인 뒤 4장 보완 문안(규칙 16), 카드 실물, push(규칙 4)에서 한 번씩 더 승인을 받는다. 로컬에만 있는 기록 커밋은 다음 push 때 함께 올린다.
...
- [ ] 3-8 (C) 소개 카드 구현. 세부 계획 작성(2026-09-15), 승인 대기
- [ ] 3-9 (C) 종단 검증
