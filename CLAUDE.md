...
> **현재 위치:** Phase 1 진행 중. Task 1~7 완료, **Task 8 코드 완료**(테스트 30개 통과).
> - 실제 실행을 시도했더니 BigQuery 물리 스토리지 확인 게이트에서 멈췄다. 설계대로 반쪽 스냅샷은 남지 않았다.
> - 사용자가 물리 스토리지 2행을 확인해 주면 실제 실행과 화면 확인을 하고 Phase 1을 마친다(규칙 10).
...
| Phase 1: 가격·TCO·대시보드 | **진행 중** (Task 8 코드 완료, 실제 실행 대기) | 테스트 30개 통과. 판정 미리보기: T1 기각(미국 민감·서울 견고), T2 지지(39.5%p). 임시 디자인 대시보드 완성 | `docs/phases/phase-1-tco-dashboard.md`, `docs/plans/phase1-tco-dashboard.md` |
...
**결정·확인 대기**
1. **BigQuery 물리 스토리지 2행 확인.** 스토리지 섹션의 "활성 물리적 스토리지"다. 기대값은 US(us) $0.000054795, 서울 $0.000071233 per GiB-시간이다. 이 확인이 끝나야 Task 8 실제 실행이 된다.
2. (U) 실제 실행 뒤 `site/index.html` 화면 확인, 자동 수집 가격 3개 대조 → Phase 1 완료
3. (Phase 2-5a) 마음에 드는 대시보드 템플릿
4. LinkedIn 방식: Phase 3에서 결정한다. A' 공식 공유 링크를 추천한다.

**다음 행동:** 사용자가 1번 결과를 주면 다음 순서로 진행한다. 그다음 Phase 2 계획 문서를 작성하고 승인받는다. 대시보드를 만들기 전(2-5a)에는 멈춘다.
1. `confirmed_on`을 기록하고 실제 실행한다.
2. 첫 스냅샷을 커밋하고 화면 확인을 요청한다.
3. 확인을 받으면 Phase 1을 완료로 기록한다.
...
| `docs/plans/phase1-tco-dashboard.md` | Phase 1 코드 단위 상세 계획(Task 1~8). 맨 위에 진행 현황과 계획 이후 변경점(물리 스토리지, T1 두 리전, 스냅샷 원자적 저장)이 있다 |
...
- **스냅샷 원칙:** 스냅샷은 모든 수집기가 성공했을 때만 저장한다. 반쪽 스냅샷은 저장하지 않는다.
- **로컬 실행:** `.venv\Scripts\python.exe pipeline.py` (수집) / `pipeline.py --offline <날짜>` (저장된 스냅샷으로 계산)
...
- [ ] **Task 4 (1-1, 1-4 변경) 수동 가격표 + 확인 게이트.** 코드 완료(커밋 `c8da16f`), 확인 10행 중 8행 완료
  ...
  - [ ] (U) **BigQuery 물리 스토리지 2행 확인**(가정 검토로 논리 → 물리로 변경). Task 8 실제 실행이 여기서 멈춘다.
...
- [x] **Task 7 (1-7) 정적 대시보드(기능 확인용 임시 디자인).** 완료, 커밋 `4e90917`, 템플릿 추적 수정 `88fdddc`
  - 사용자 결정에 따라 T1을 **미국·서울 두 리전 모두** 표시한다. `render()`는 `t1_by_region`을 받는다.
  - 테스트 2개 통과(누적 28개).
  - `.gitignore`의 `site/`가 `templates/site/`까지 제외한 사고가 있었다. `/site/`로 고쳤다.
- [ ] **Task 8 (1-8) `pipeline.py` 실제 실행과 검증.** 코드 완료, 실제 실행 대기
  - [x] `pipeline.py` 작성. T1 두 리전을 넘기고, 세 수집기가 모두 성공한 뒤에만 스냅샷을 저장한다.
  - [x] 테스트 2개 통과(누적 **30개**): 오프라인 실행, 게이트 중단 시 반쪽 스냅샷 없음
  - [x] 실제 실행 시도: AWS·Azure 수집 후 BigQuery 물리 스토리지 게이트에서 멈췄고, `data/raw/`·`site/`는 생기지 않았다(정상)
  - [ ] (U) 물리 스토리지 확인 → `confirmed_on` 기록 → 실제 실행 → 첫 스냅샷 커밋
  - [ ] (U) `site/index.html` 화면 확인, 자동 수집 가격 3개 대조
...
| 2026-09-11 | 파이프라인은 **세 수집기가 모두 성공한 뒤에만** 스냅샷을 저장한다(테스트 추가, Phase 1 합계 30개) | 게이트에서 멈출 때 반쪽 스냅샷이 남으면 Phase 2 변동 감지가 오염된다 |
...
- **2026-09-11:** Task 7을 완료했다.
  ...
  - 커밋은 `4e90917`이다.
  - **사고와 수정 (커밋 `88fdddc`)**
    - `.gitignore`의 `site/` 규칙이 `templates/site/`까지 제외해서, 템플릿이 첫 커밋에서 빠졌다.
    - 규칙을 루트 전용 `/site/`로 고치고 템플릿을 추가했다.
    - 교훈: 커밋 뒤에는 `git ls-files`로 새 파일이 실제로 추적되는지 확인한다(규칙 11에 추가).
- **2026-09-11:** Task 8 코드를 완료했다.
  - `pipeline.py`를 TDD로 구현했다(ImportError로 실패 확인 → 구현 → 통과).
  - 테스트는 누적 **30개**가 통과했다.
  - 계획에서 바꾼 점: 세 수집기가 모두 성공한 뒤에만 스냅샷을 저장한다(반쪽 스냅샷 방지 테스트 추가).
  - 실제 실행을 시도했다. AWS·Azure는 수집됐고, BigQuery 물리 스토리지 게이트에서 멈췄다. `data/raw/`와 `site/`는 생기지 않았다. 설계대로다.
  - 사용자의 물리 스토리지 확인을 기다린다. 확인되면 실제 실행 → 화면 확인 → Phase 1 완료 순서로 간다.
