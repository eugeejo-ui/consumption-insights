...
>   - **Phase 3 진행 중:** 방식 A안, 계획 3건 승인. 3-1(굽기 실측)·3-2(카드 스크립트, 2026-09-14 개정본 확정) 완료. 3-3(카드 데이터·게시문, `06ace44`)·3-4(카드 템플릿·굽기, `d2490bc`)·3-5(파이프라인 연결, `6a8b023`, 테스트 101개) 완료(2026-09-15).
...
| Phase 3: LinkedIn 카드뉴스 | **진행 중** (3-1~3-5 완료, 3-6 구현 완료·push 승인 대기, 테스트 108개, 2026-09-15) | 방식 A안 확정(사람이 게시), 디자인 템플릿 수령·분석 완료. 카드는 1080×1350, 소개 5장·정기 5장 이상(마무리 안내 장 포함, D20) | `docs/phases/phase-3-linkedin.md`, `docs/plans/phase3-card-design.md`, `docs/plans/phase3-linkedin-cardnews.md` |
...
**다음 행동:** 3-6 구현 완료(`d0529c7`, 테스트 108개, 회귀 수정 포함). **외부 행동 3건의 사용자 승인을 기다린다**(규칙 4): main push(커밋 약 21개, push 시 게시 워크플로가 대시보드를 다시 배포), 수집 워크플로 시뮬레이션 실행, 확인 뒤 시뮬레이션 PR 닫기.
...
- [x] 3-3 (C) 카드 데이터와 게시문 `publish/card_data.py`, `publish/render_linkedin.py` (2026-09-15, `06ace44`, 테스트 79개). 누락·숫자·글자 수·게시문 길이 검사를 코드로 강제한다
- [x] 3-4 (C) 카드 HTML 템플릿과 굽기 → (U) 카드 실물 확인 (2026-09-15, `d2490bc`, 테스트 92개). 겹침·넘침·줄 수·한 음절 줄을 브라우저 실측으로 검사한다
- [x] 3-5 (C) 파이프라인 연결 `pipeline.py --cards` (2026-09-15, `6a8b023`, 테스트 101개). 카드 실패가 검토 자료를 막지 않고, 승인된 날의 카드만 사이트에 복사한다
- [ ] 3-6 (C) 워크플로 연결(검토 PR에 카드 표시, 머지 후 게시). 구현 완료(`d0529c7`, 테스트 108개), push·시뮬레이션 승인 대기
...
- **2026-09-15:** 사용자가 보완 문안을 승인했다. **3-3을 완료했다**(`06ace44`, 테스트 62 → 79). 카드 데이터와 게시문 텍스트를 만들고, 누락·숫자·글자 수·게시문 길이 검사를 코드로 강제한다. 실제 승인 스냅샷 시뮬레이션에서 5장(마무리 안내 포함)과 게시문 371자가 나왔고, 차트 금액이 Task 2 시험 카드와 같다 [확인]. 다음은 3-4다.
...
- **2026-09-15:** 3-4 계획과 디자인 조정안 6건을 승인받아 구현했다(`d2490bc`, 테스트 79 → 92). 레이아웃 측정값은 `--dump-dom`으로 받는다(PDF 제목 방식은 4,096자에서 잘린다) [확인]. 실제 승인 스냅샷으로 5장·10장 두 벌을 구워 레이아웃 검사를 통과했다. 차트 장 각주-알약 여백이 30px에서 131px로 늘었다. 사용자 실물 확인을 기다린다.
...
- **2026-09-15:** **3-5 완료**(`6a8b023`, 테스트 92 → 101). E1인 날 1단계가 카드와 게시문을 만들고, 실패는 `card-errors.txt`와 단계 출력에 남긴다. 승인된 날의 카드만 `site/cards/`로 복사한다. 굽기 전에 한글 글꼴을 확인한다. 종단 실측 중 상대 경로로 굽으면 Edge가 파일을 쓰지 못하는 결함을 발견해 고쳤다 [확인]. 다음은 3-6이다.
...
- **2026-09-15:** 검토 PR 본문 문안이 승인됐다. 3-6을 구현했다(`d0529c7`, 테스트 101 → 108). 검토 PR 본문을 `publish/review_pr.py`로 조립하고, CI에 한글 글꼴을 pytest보다 먼저 설치한다. 3-5 회귀(게시 워크플로의 승인일 파싱)를 단계 출력으로 고쳤다. push 범위 20개 커밋에서 개인정보 0건, 원격 봇 커밋과 겹치는 파일 0개를 확인했다. push·시뮬레이션 실행 승인을 기다린다.
