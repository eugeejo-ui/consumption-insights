...
**다음 행동:** 3-7 세부 계획(`docs/tasks/phase3/task-7-revise-loop.md`)을 작성했고 **승인을 기다린다.** 승인 뒤 새 문안(규칙 16)과 push·실측(규칙 4)에서 한 번씩 더 승인을 받는다. 로컬에만 있는 기록 커밋은 다음 push 때 함께 올린다.
...
- [ ] 3-7 (C) **문구 반려 재작성 루프**(규칙 16). `revise-copy` 라벨 → 재작성 요청 → humanize-korean → 같은 PR 갱신. 세부 계획 작성(2026-09-15), 승인 대기
- [ ] 3-8 (C) 소개 카드 구현
- [ ] 3-9 (C) 종단 검증
