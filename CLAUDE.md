...
> **현재 위치:** Phase 1 진행 중. **Task 1~6 완료.** Task 4는 BigQuery 물리 스토리지 2행이 확인 대기다. 다음은 Task 7(정적 대시보드)이고, 사용자 지시를 기다린다(규칙 10). Phase별 진행계획은 `docs/phases/`에 있다.
> **마지막 갱신:** 2026-09-11
...
| Phase 1: 가격·TCO·대시보드 | **진행 중** (Task 6/8 완료) | 테스트 26/28 통과. 판정 미리보기: **T1 기각**(전 시나리오 Redshift 1위, 미국은 민감·서울은 견고), **T2 지지**(39.5%p) | `docs/phases/phase-1-tco-dashboard.md`, `docs/plans/phase1-tco-dashboard.md` |
...
**결정·확인 대기**
1. **Task 7 착수 지시, 그리고 대시보드의 T1 표시 범위.** 계획은 미국만이다. 리전마다 견고성이 달라서 미국·서울 모두 표시하는 쪽을 제안한다.
2. **BigQuery 물리 스토리지 2행 확인.** 스토리지 섹션의 "활성 물리적 스토리지"다. 기대값은 US(us) $0.000054795, 서울 $0.000071233 per GiB-시간이다. Task 8 전까지 필요하다.
3. LinkedIn 방식: Phase 3에서 결정한다. A' 공식 공유 링크를 추천한다.

**다음 행동:** 사용자가 지시하면 Task 7(정적 대시보드)을 한다. 1번 결정을 반영한다. 끝나면 보고하고 멈춘다.
...
| `docs/phases/phase-1-tco-dashboard.md` | Phase 1 진행계획. Task 표, 가격 확인 상태, 가정 검토 결과, **T1·T2 판정 결과**, 사용자 할 일, 위험, 진행 기록 |
...
- [x] **Task 6 (1-6) T1 순위·민감도, T2 서울 프리미엄 판정.** 완료
  - `config/thresholds.yaml`을 만들고, 판정 함수 4개를 추가했다.
  - 테스트 6개 통과(누적 26개).
  - 실제 가격 기준 미리보기
    - T1: 미국·서울 모두 기각(전 시나리오 Redshift 1위). 미국은 **민감**, 서울은 **견고**
    - T2: 지지(39.5%p)
- [ ] Task 7 (1-7) 정적 대시보드: 판정표, 차트, 가정 공개, 가격 확인일. **다음 차례.** T1 표시 범위(미국만 / 두 리전) 결정 필요
...
- **2026-09-11:** Task 6을 완료했다.
  ...
  - 실제 가격 기준 판정(미리보기)
    ...
    - T2는 지지(39.5%p)
  - Task 7 관련해 T1을 두 리전 모두 표시하자는 제안을 남겼다.
  - 다음 지시를 기다린다.
