...
>   - **Phase 3 진행 중:** 방식 A안, 계획 3건 승인. 3-1(굽기 실측)·3-2(카드 스크립트, 2026-09-14 개정본 확정) 완료. 3-3(카드 데이터·게시문, `9f50492`)·3-4(카드 템플릿·굽기, `88174ca`)·3-5(파이프라인 연결, `b2c8576`)·3-6(워크플로 연결, `d72560e`)·3-7(문구 반려 루프, `5f8740f`, 테스트 117개) 완료(2026-09-15). Task 3~7 코드를 push했다.
...
| Phase 3: LinkedIn 카드뉴스 | **진행 중** (3-1~3-7 완료, 테스트 117개, 2026-09-15) | 방식 A안 확정(사람이 게시), 디자인 템플릿 수령·분석 완료. 카드는 1080×1350, 소개 5장·정기 5장 이상(마무리 안내 장 포함, D20) | `docs/phases/phase-3-linkedin.md`, `docs/plans/phase3-card-design.md`, `docs/plans/phase3-linkedin-cardnews.md` |
...
**다음 행동:** 3-8을 로컬에서 구현하고 소개 카드 5장을 구웠다(테스트 127개). 사용자가 카드 실물을 승인했다. **push 승인을 기다린다**(규칙 4). push 뒤 게시 주소에서 소개 카드를 확인하고 3-8을 마친다.
...
- [ ] 3-8 (C) 소개 카드 구현. 계획·D21·D22·4장 문안·카드 실물 승인, 로컬 커밋(테스트 127개), push 승인 대기(2026-09-15)
- [ ] 3-9 (C) 종단 검증
