...
**다음 행동:** 3-6 세부 계획(`docs/tasks/phase3/task-6-workflow.md`)을 작성했고 **승인을 기다린다.** 승인 뒤 검토 PR 본문 문안(규칙 16)과 push·시뮬레이션 실행(규칙 4)에서 한 번씩 더 승인을 받는다. 3-5의 회귀 결함(게시 워크플로가 승인일을 읽지 못함)을 push 전에 고친다.
...
- [ ] 3-6 (C) 워크플로 연결(검토 PR에 카드 표시, 머지 후 게시). 세부 계획 작성(2026-09-15), 승인 대기
