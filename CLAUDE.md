...
> **현재 위치:** Phase 1 진행 중. Task 1~7 완료, Task 8 코드 완료(테스트 30개). 수동 가격 10행을 모두 확인했다(2026-09-11).
> - **사용자 요청으로 중간 검토 체크포인트(Task 8b)를 설계했고, 승인을 기다린다.**
> - 승인되면 구현한다. 그다음 1단계를 실제 실행해 검토 보고서를 드리고 멈춘다.
...
| Phase 1: 가격·TCO·대시보드 | **진행 중** (Task 8b 설계 승인 대기) | 테스트 30개 통과, 수동 가격 10행 확인 완료. 판정 미리보기: T1 기각(미국 민감·서울 견고), T2 지지(39.5%p) | `docs/phases/phase-1-tco-dashboard.md`, `docs/plans/phase1-tco-dashboard.md` |
| Phase 2: 자동 수집·검토 PR·게시·Sheets 적재 | 대기 | 체크포인트 = 검토 PR 머지. Sheets 적재는 컨펌 이후. 대시보드를 만들기 전에 멈추고 사용자 템플릿을 받는다(2-5a) | `docs/phases/phase-2-automation-publish.md` |
...
**결정·확인 대기**
1. **Task 8b 중간 검토 체크포인트 설계 승인.** 1단계에서 검토 보고서까지 만들고 멈추고, 컨펌하면 2단계 `--confirm`으로 화면을 생성한다. 테스트는 33개가 된다.
2. (U) 1단계 실행 뒤 검토 보고서 판단 → 컨펌 → 2단계 뒤 화면 확인 → Phase 1 완료
3. (Phase 2-5a) 마음에 드는 대시보드 템플릿
4. LinkedIn 방식: Phase 3에서 결정한다. A' 공식 공유 링크를 추천한다.

**다음 행동:** 사용자가 1번을 승인하면 다음 순서로 진행한다.
1. Task 8b를 TDD로 구현하고 커밋한다.
2. 1단계를 실제로 실행해 검토 보고서를 드리고 **멈춘다**.
3. 컨펌을 받으면 2단계를 실행하고, 첫 스냅샷을 커밋한 뒤 화면 확인을 요청한다.
...
14. **중간 검토 체크포인트: 파이프라인은 중간 결과를 만든 뒤 멈추고, 사용자 컨펌 후에만 나머지를 진행한다** (2026-09-11 사용자 지시).
    - 로컬(Phase 1): `pipeline.py`는 검토 보고서까지 만들고, `pipeline.py --confirm <날짜>`로 반영한다.
    - 자동(Phase 2): 검토 PR을 머지하는 것이 컨펌이다.
...
| `docs/phases/phase-1-tco-dashboard.md` | Phase 1 진행계획. Task 표, 가격 확인 상태, **중간 검토 체크포인트(Task 8b)**, 가정 검토, 판정 결과, 대시보드 구성, 위험, 진행 기록 |
...
| `docs/plans/phase1-tco-dashboard.md` | Phase 1 코드 단위 상세 계획(Task 1~8). 맨 위에 계획 이후 변경점과 **Task 8b 설계(인터페이스, 테스트 목록)** 가 있다 |
...
```
[Phase 1 로컬]  pipeline.py → 수집·스냅샷·계산 → review.md → [멈춤: 사용자 판단] → pipeline.py --confirm <날짜> → site/ + 스냅샷 커밋
...
- **스냅샷 원칙:** 모든 수집기가 성공했을 때만 저장한다. 컨펌한 스냅샷만 커밋하고, 비교 기준으로도 쓴다.
...
- [x] **Task 1 (1-0) 뼈대 + 공통 스키마.** 완료, 커밋 `d5930d1`. 테스트 3개, noreply 이메일 첫 커밋
- [x] **Task 2 (1-2) AWS Redshift 수집기.** 완료, 커밋 `0c5b6c9`
  - 테스트 누적 6개.
  - 실제 조회값은 서버리스 RPU $0.375/$0.438, 관리형 스토리지 $0.024/$0.0261(미국/서울)이다 [확인].
- [x] **Task 3 (1-3) Azure 수집기.** 완료, 커밋 `4e2ef6c`
  - 테스트 누적 9개.
  - 실제 조회값은 서버리스 SQL DBU $0.70/$0.95, ADLS $0.0208/$0.02(미국/서울)이다 [확인].
- [x] **Task 4 (1-1, 1-4 변경) 수동 가격표 + 확인 게이트.** 완료, 커밋 `c8da16f`
  - 테스트 누적 13개.
  - **수동 가격 10행 모두 확인(2026-09-11).** 대상은 Snowflake 4행, BigQuery 컴퓨트 4행, BigQuery 물리 스토리지 2행이다.
- [x] **Task 5 (1-5) 워크로드 가정 + 월 비용 계산.** 완료, 커밋 `56e95fb`, 가정 검토 반영 `529fbc4`. 테스트 누적 20개
- [x] **Task 6 (1-6) T1·T2 판정.** 완료, 커밋 `7126385`
  - 테스트 누적 26개.
  - 미리보기: T1 기각(미국 민감, 서울 견고), T2 지지(39.5%p)
- [x] **Task 7 (1-7) 정적 대시보드(임시 디자인).** 완료, 커밋 `4e90917`, `88fdddc`
  - T1을 두 리전 모두 표시한다. 테스트 누적 28개.
- [x] **Task 8 (1-8) `pipeline.py`.** 코드 완료, 커밋 `e1be955`
  - 테스트 누적 30개.
  - 세 수집기가 모두 성공한 뒤에만 스냅샷을 저장한다.
- [ ] **Task 8b 중간 검토 체크포인트 (사용자 요청, 규칙 14).** **설계 승인 대기**
  - [ ] `publish/review_report.py`, `templates/review.md.j2`, `pipeline.py` 수정(기본 실행은 검토 보고서까지, `--confirm DAY`로 반영)
  - [ ] 테스트 33개 통과
  - [ ] 1단계 실제 실행 → 검토 보고서 전달 → **멈춤**
  - [ ] (U) 검토 보고서 판단·컨펌
  - [ ] 2단계 실행 → `site/index.html` 생성·`approved.txt` 기록 → 첫 스냅샷 커밋
  - [ ] (U) 화면 확인, 자동 수집 가격 3개 대조
- **게이트:** (U) 화면 확인 → Phase 2 상세 계획 승인(2-5a 전 멈춤)
...
- [ ] 2-2 (C) `detect/diff.py`
  - **이전 승인 스냅샷**과 비교해 `events.json`을 만든다.
...
| 2026-09-11 | **중간 검토 체크포인트를 둔다**(규칙 14). Phase 1은 검토 보고서 → `--confirm`, Phase 2는 검토 PR 머지가 컨펌. Sheets 적재는 컨펌 이후로 옮긴다 | 사용자 지시("중간 결과를 내가 판단해서 컨펌하면 나머지 진행"). **Task 8b 설계 승인 대기** |
...
- **2026-09-11:** Task 6을 완료했다(`7126385`). 미리보기에서 T1은 기각(미국 민감, 서울 견고), T2는 지지(39.5%p).
- **2026-09-11:** Task 7을 완료했다(`4e90917`). T1을 두 리전 모두 표시한다. 규칙 13을 추가했다. `.gitignore` 사고를 `/site/`로 수정했다(`88fdddc`).
- **2026-09-11:** Task 8 코드를 완료했다(`e1be955`). 테스트 30개. 세 수집기가 모두 성공한 뒤에만 저장한다. 실제 실행은 물리 스토리지 게이트에서 멈췄고, 반쪽 스냅샷은 남지 않았다.
- **2026-09-11:** BigQuery 물리 스토리지를 사용자가 확인했다($0.04/$0.052, 월 기준 표시값도 일치). 수동 가격 10행이 모두 확인됐다.
  - 사용자 요청에 따라 중간 검토 체크포인트(규칙 14, Task 8b)를 설계했다.
    - Phase 1은 `review.md` → `--confirm`이다.
    - Phase 2는 검토 PR 머지가 컨펌이고, Sheets 적재는 컨펌 이후다.
  - 설계 승인을 기다린다.
