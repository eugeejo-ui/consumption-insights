...
>   - **Phase 3 진행 중:** 방식 A안, 계획 3건 승인. 3-1(굽기 실측)·3-2(카드 스크립트, 2026-09-14 개정본 확정) 완료. 3-3(카드 데이터·게시문, `9f50492`)·3-4(카드 템플릿·굽기, `88174ca`)·3-5(파이프라인 연결, `b2c8576`)·3-6(워크플로 연결, `d72560e`, 테스트 108개) 완료(2026-09-15). Task 3~6 코드를 push했다.
...
| Phase 3: LinkedIn 카드뉴스 | **진행 중** (3-1~3-6 완료, 테스트 108개, 2026-09-15) | 방식 A안 확정(사람이 게시), 디자인 템플릿 수령·분석 완료. 카드는 1080×1350, 소개 5장·정기 5장 이상(마무리 안내 장 포함, D20) | `docs/phases/phase-3-linkedin.md`, `docs/plans/phase3-card-design.md`, `docs/plans/phase3-linkedin-cardnews.md` |
...
**다음 행동:** 3-7을 로컬에서 구현했다(테스트 117개). **push와 CI 실측(시뮬레이션 PR에 라벨 붙이기·떼기) 승인을 기다린다**(규칙 4). 로컬에만 있는 커밋은 그때 함께 올린다.
...
- [ ] 3-7 (C) **문구 반려 재작성 루프**(규칙 16). `revise-copy` 라벨 → 재작성 요청 → humanize-korean → 같은 PR 갱신. 계획·새 문안 승인, 로컬 구현(테스트 117개), push·CI 실측 승인 대기(2026-09-15)
- [ ] 3-8 (C) 소개 카드 구현
- [ ] 3-9 (C) 종단 검증
