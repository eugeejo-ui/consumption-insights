...
- **2026-09-15 (Phase 3 Task 9):** 게시 워크플로의 시트 적재 조건을 고쳤다. 이전에는 최근 승인일의 `events.json`으로 적재해, 첫 실제 변동을 승인한 뒤 main에 코드를 push할 때마다 같은 행이 다시 쌓일 수 있었다(코드 확인, 실제 피해 없음). 이제 승인 기록 단계가 그 실행에서 새로 승인한 날짜를 단계 출력(`confirmed`)으로 내고, 적재는 그 날짜에만 한다.
