...
- [ ] **Step 1** 테스트 3개를 먼저 쓴다(실패 확인)
- [ ] **Step 2** `publish/browser.py`를 구현한다 → 테스트 3개 통과, 전체 60개 통과
- [ ] **Step 3** 시험용 HTML 1장을 스크래치에 만든다
  - 담을 것: 한글 초대형 제목, 그라데이션 단어, 회색 본문, 라운드 카드, CSS 막대 4개, 상·하단 바
  - **외부 요청 0건**으로 만든다(글꼴은 시스템 스택)
- [ ] **Step 4** 로컬에서 PNG와 PDF를 굽는다
  - PNG 크기를 PNG 헤더(IHDR)에서 직접 읽어 1080×1350인지 확인한다(Pillow를 쓰지 않는다)
  - PDF 쪽수를 바이트에서 확인한다
  - **사용자에게 PNG를 보내 눈으로 확인받는다**
- [ ] **Step 5** CI 실측용 임시 워크플로를 만든다(`workflow_dispatch` 전용)
...
- 로컬에서 1080×1350 PNG와 1쪽 PDF가 나오고, 한글과 그라데이션이 정상이다(사용자 확인).
- CI 서버에서도 같은 결과가 나온다.
- 테스트 60개가 통과한다.
...
> 아직 실행하지 않았다. 승인 후 이 절을 채운다(규칙 15).
