...
Task 2 개정 중 시험 굽기가 연속으로 실패했다.

1. 첫 실행: `PermissionError: CrashpadMetrics-active.pma` — 임시 프로필 폴더 삭제 단계에서 예외
2. 삭제 예외를 무시하도록 고친 뒤: `파일이 생기지 않았다` — 두 가지 헤드리스 옵션 모두 실패
3. 실제 브라우저 테스트 `test_bakes_a_png_of_the_right_size`도 실패

### 원인 [확인]

**Edge 실행 파일이 먼저 종료 코드 0을 돌려주고, 실제 굽기는 분리된 하위 프로세스가 몇 초 뒤에 끝낸다.** 표준 출력과 표준 오류는 비어 있었다. 실패로 판정된 직후 폴더를 확인하니 결과 PNG가 뒤늦게 생성돼 있었다.

- 코드는 종료 직후 파일 유무를 확인했으므로 "파일이 생기지 않았다"로 판정했다.
- 같은 이유로, 하위 프로세스가 아직 쓰고 있는 프로필 폴더를 지우려다 잠금 오류가 났다.
- 2026-09-12 실측에서는 동기적으로 동작해 드러나지 않았다. 사용자 브라우저에 떠 있는 헤드리스 프로세스는 없었고, 사용자 브라우저 프로세스는 건드리지 않았다.

### 수정

| 항목 | 내용 |
|---|---|
| `wait_for_file(out, timeout)` | 파일이 생기고 크기가 두 번 연속 같으면 완료로 본다. 0.5초 간격, 상한 60초(`FILE_WAIT`) |
| `_attempt(args, out)` | 브라우저 종료 뒤 **프로필 폴더가 살아 있는 동안** 파일을 기다린다. 그다음 폴더를 지운다 |
| `TemporaryDirectory(ignore_cleanup_errors=True)` | 충돌 보고 프로세스의 잠금이 남아도 굽기 실패로 보지 않는다 |
| 테스트 +2 | 종료 뒤 늦게 쓰이는 파일을 기다리는지, 끝내 파일이 없으면 포기하는지 |

**결과:** 테스트 62개 통과(실제 브라우저 굽기 테스트 포함) [확인]. 두 리전 차트 시험 카드 2장을 정상적으로 구웠다.

### 남는 것

- 삭제에 실패한 임시 프로필 폴더가 `%TEMP%`에 남을 수 있다. 굽기 결과에는 영향이 없다.
- CI(Chrome, Linux)에서는 같은 현상이 확인되지 않았다. 대기 로직은 동기 동작에도 해가 없다(파일이 이미 있으면 1초 안에 끝난다).
